...
/*-----------------------------------------Timmer-----------------------------------------*/
    ...
    Timmer::Timmer(int timeInterval)//分辨率
        : timeNum_(1'000 / timeInterval),
          timeWheel_(timeNum_),
          mutexVec(timeNum_),
          wheelIndex_(0),
          working_(false)
        ...
        while (working_)
        {
            ...
            for (auto iter = wheelList.begin(); iter != wheelList.end(); iter++)
            {
                ...
                if (timer->timePoint() <= timeBegin)
                {
                    ...
                    iter = wheelList.erase(iter); //小心这里迭代器失效的问题,返回后继元素迭代器
                }
            }
