...
namespace Time
{
    ...
    protected:
        long timePoint_;  //记录应该执行的时间    mS
        long insertTime_; //记录放入定时器中的时间 mS
        bool repeatable_; //是否是重复事件
        int id_; //便于以后用来删除,用于周期任务
    };
    ...
    {
    public:
        using timeDuration = std::chrono::duration<int, std::milli>;//ms
        static Timmer* init(int);
        Timmer(const Timmer& another) = delete;//不允许复制
    ...
    private:
        ...
        std::mutex priodTimerGuarder_;//周期类任务的互斥锁
