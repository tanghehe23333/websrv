...
namespace Log
{    
    const int LogStream::maxNumLength=48;
   // const short LogStream::maxLineSize=128;
    ...
    template <typename T>
    size_t LogStream::convert(char buf[], T value)//可改
    {
        T i = value;
        char *p = buf;
        do
    ...
    LogStream &LogStream::operator<<(int value)
    {
        ...
        assert(size <= maxNumLength);

        streamBuf_.writeIn(buf, size);
        return *this;
    }
    ...
    {
        char buf[maxNumLength] = {0};
        int size = snprintf(buf, maxNumLength, "%.12g", value);
        streamBuf_.writeIn(buf, size);
        return *this;
    }
    ...
    {
        bool writeEndSuccess = stream.streamBuf_.writeEnd(); 
        assert(stream.updateCallBack_);
        if(writeEndSuccess)
        {
            auto [streamBuf, index] = stream.streamBuf_.updateMes();
            stream.updateCallBack_(streamBuf, index);
            stream.streamBuf_.refresh();
        }
        return stream;
    }
}